...
//Days Calculation Purpose added by vtTigress Team
...
$server->register(
	...
	array('id'=>'xsd:string','block'=>'xsd:string','sessionid'=>'xsd:string','only_mine'=>'xsd:string','category'=>'xsd:string'),
	array('return'=>'tns:field_details_array'),
	$NAMESPACE);
...
$server->register(
	...
	array('return'=>'tns:common_array'),
	$NAMESPACE);
$server->register('get_quotes_count',array('fieldname'=>'tns:common_array'),
	array('return'=>'tns:common_array'),
	$NAMESPACE);
/**
...
{ 
	...
	$accountname = $input_array['company_name'];
	$mailingcity = $input_array['mailingcity'];
	$mailingcountry = $input_array['mailingcountry'];
	
	$sql = "SELECT * FROM vtiger_contactdetails WHERE email = '$pemail'";
	...
	$orgsql = "SELECT vtiger_account.*, vtiger_accountbillads.bill_city FROM vtiger_account
	INNER JOIN vtiger_accountbillads ON vtiger_accountbillads.accountaddressid = vtiger_account.accountid
	 WHERE vtiger_account.accountname = '$accountname' AND vtiger_accountbillads.bill_city ='$mailingcity' AND vtiger_accountbillads.bill_country ='$mailingcountry' ";

	 $orgresult = $adb->pquery($orgsql, array());
	$orgnoofrows = $adb->num_rows($orgresult);

	if($orgnoofrows != 0){
		$output[0]['status'] = false;
		$output[0]['err'] = "company";
		$output[0]['msg'] = "Your company name existed";
		return $output;
	}
	...
	$org = new Accounts();
	$org->column_fields['accountname'] = $input_array['company_name'];
	$org->column_fields['email1'] = $input_array['email'];
	$org->column_fields['assigned_user_id'] = $user_id;
	$org->column_fields['cf_1014'] = 0;
	$org->column_fields['bill_city'] = $input_array['mailingcity'];
	$org->column_fields['bill_state'] = $input_array['mailingstate'];
	$org->column_fields['bill_country'] = $input_array['mailingcountry'];
	$org->save('Accounts');
	...
	$focus->column_fields['account_id'] = $org->id;  
...
function user_unique($input_array)
{ 
...
function Update_Contact($input_array){
		...
		$id = $input_array['id'];
		$userfile = $user->retrieveCurrentUserInfoFromFile($defaultAssignee);
		$contactid = vtws_getWebserviceEntityId('Contacts', $id);
		if (!empty($contactid)) {
			$data = vtws_retrieve($contactid, $userfile);
		}
		$wsid = vtws_getWebserviceEntityId('Users', $defaultAssignee);
		$excluded = array('id','module');
		foreach($input_array as $k=>$v){
			if(!in_array($k,$excluded)){
				$data[$k]=vtlib_purify($v);
			}
		}
		vtws_update($data, $userfile);		
	$record_array[0]['contactid'] = $id;
	return $record_array;
}
...
function getImagePath($id)
	{	
		global $adb;
		$sql = "SELECT * FROM vtiger_attachments INNER JOIN  vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid WHERE vtiger_seattachmentsrel.crmid =$id";
		$result = $adb->pquery($sql, array());
		if($adb->num_rows($result) >= 1){
			...
			return $finaldata[0]['path'].$finaldata[0]['attachmentsid'].'_'.$finaldata[0]['name'];
		}else{
	...
	{	
		global $adb;
		$sql = "SELECT * FROM vtiger_attachments WHERE vtiger_attachments.attachmentsid =$id";
		$result = $adb->pquery($sql, array());
		if($adb->num_rows($result) >= 1){
			$finaldata =  array();
			while($data = $adb->fetchByAssoc($result)){
			...
			return "public.php?fid=".$finaldata[0]['attachmentsid'].'&key='.$finaldata[0]['name'];
		}else{
			return "";
		}
...
//Dashboard Count Purpose 
function get_quotes_count($id,$only_mine='true')
{
	$modulename = "Quotes";	
	global $current_user,$adb,$log;
	$log->debug("Entering customer portal function get_quotes_count ..");
	...
	$quote_count = array();
	$query = "select distinct vtiger_quotes.*,vtiger_crmentity.*,vtiger_quotesbillads.*,vtiger_quotesshipads.*,
	...
	//Standard Quotes :: Published Quotes cf_1446
	$sql1 = $query." and cf_1446 = 1 ";
	$quote_count['standard_count'] = $adb->num_rows($adb->pquery($sql1,array()));
	//TDU Count -- cf_1564
	$sql2 = $query." and cf_1564 = 1 ";
	$quote_count['tdu_count'] = $adb->num_rows($adb->pquery($sql2,array()));
	//Custom Count
	$sql3 = $query." and (accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
		$params = array($entity_ids_list,$entity_ids_list);
	$quote_count['custom_count'] = $adb->num_rows($adb->pquery($sql3,$params));
	return array($quote_count);
}
//Ended here

function get_quotes($id,$only_mine='true',$standard = false,$tdu =false)
{
	$modulename = "Quotes";
	require_once('modules/Products/Products.php');
	require_once('include/utils/UserInfoUtil.php');
	global $current_user,$adb,$log;
	$log->debug("Entering customer portal function get_quotes ..");
	$check = checkModuleActive($modulename);
	if($check == false){
		return array("#MODULE INACTIVE#");
	}
	...
	if($tdu){
		$query .=" and vtiger_quotescf.cf_1564=1";
		$params = array();
	}elseif($standard){
		$query .=" and vtiger_quotescf.cf_1446=1";
		$params = array();
	}else{
		$query .=" and (accountid in  (". generateQuestionMarks($entity_ids_list) .") or contactid in (". generateQuestionMarks($entity_ids_list) ."))";
		$params = array($entity_ids_list,$entity_ids_list);
	...
	$fieldres = $adb->pquery($fieldquery,array(getTabid($modulename)));
	$nooffields = $adb->num_rows($fieldres);
	while($row = $adb->fetchByAssoc($fieldres)){
		$fields_list[$row['fieldlabel']] =$row['fieldname'];
	}
	$fieldValuesToRound = array('unit_price','weight','commissionrate','qtyinstock');
		...
		{
			$prodcutsinfo = array();
			$fieldid = $adb->query_result($res,$j,'crmid');
			foreach($fields_list as $fieldlabel=> $fieldname) {
				...
				if($fieldname == 'unit_price'){
					$sym = '';//getCurrencySymbol($res,$j,'currency_id'); Todo for Vtigress Team
					$fieldvalue = $sym.$fieldvalue;
				}				
				$prodcutsinfo[$fieldname] = $fieldvalue;				
			}
			$prodcutsinfo['record'] =  $adb->query_result($res,$j,'crmid');
			$output[$modulename][$j] = $prodcutsinfo;
	}
...
function add_travelquote_record($input_array){
	global $adb,$log;
	$log->debug("Entered add_quote_so_inv_record ");
	require_once 'modules/Contacts/models/Record.php';
	require_once 'modules/Vtiger/models/Record.php';
	include_once 'include/Webservices/Create.php';
	include_once 'include/Webservices/Update.php';
	include_once 'include/Webservices/Retrieve.php';
	
	//Contacts data prepopulate
	$contactid = $input_array['contactid'];
	$travel_Date = $input_array['travel_Date'];	
	$parentRecordModel = Contacts_Record_Model::getInstanceById($contactid,'Contacts');
	...
	foreach ($fieldMappingList as $fieldMapping) {
		...
		$customerinfo[$inventoryField] = $parentRecordModel->get($parentField);
	}	
	$user = new Users();	
	//Taxes values to pass 
	require_once 'include/utils/InventoryUtils.php';
	$taxes =getAllTaxes('available');
	$shippingTaxes =getAllTaxes('available','sh');
	$shippingtax = 0;
	foreach($shippingTaxes as $shk=>$shv){
		$shippingtax += $shv['percentage'];
	}
	$taxinfo = array();
	$totalproducts = $input_array['packageinfo']['productid'];
	foreach($taxes as $k=>$tdet){
		$taxinfo[$tdet['taxname']]=$tdet['percentage'];
	}
	$noofchild = $input_array['noofchild'];
	$noofadult = $input_array['noofadult'];
		
	foreach($totalproducts as $key=>$pid){
		$days = $input_array['packageinfo']['day'][$key];		
		$query = "SELECT vtiger_products.*,vtiger_productcf.*,vtiger_crmentity.* FROM vtiger_products INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_products.productid  LEFT JOIN vtiger_productcf  ON vtiger_productcf.productid = vtiger_products.productid LEFT JOIN vtiger_vendor ON vtiger_vendor.vendorid = vtiger_products.vendor_id WHERE vtiger_products.productid =? AND vtiger_crmentity.deleted = 0";
		$productinfo=$adb->fetch_row($adb->pquery($query,array($pid)));		
		$proid = vtws_getWebserviceEntityId('Products', $pid);
		$indate =  array_map('__vt_add_days', array(array($travel_Date,$days)));
		$child_price = $productinfo['cf_1168'];
		$infant_price = $productinfo['cf_1270'];
		$city = $productinfo['cf_1248'];
		$category = $productinfo['cf_928'];
		$lineItem[] = array_merge(array('productid'=>$proid,'noofadult1'=>$noofadult,'noofchild'=>$noofchild,'listprice'=>$productinfo['unit_price'],'cf_1168'=>$child_price,'cf_1270'=>$infant_price,'day'=>$days,'indate'=>$indate[0],'cf_928'=>$category,'city'=>$city,'ageofchild'=>0,'noofadultsinglesharing'=>$noofadult,'noofroomsinglesharing'=>$noofadult,'quantity'=>$noofadult),$taxinfo);
	}
	$subject = 'Australia Tour '.$travel_Date;
	$current_user = $user->retrieveCurrentUserInfoFromFile(Users::getActiveAdminId());
	$passengerinfo = $input_array['passengerinfo'];
	$terms_conditions = getTermsAndConditions('Quotes');//Settings_Vtiger_MenuItem_Model::getInstance("INVENTORYTERMSANDCONDITIONS");
	try {
		$defaultAssignee = getDefaultAssigneeId();
		$userid = vtws_getWebserviceEntityId('Users', $defaultAssignee); 
		$currencyid = vtws_getWebserviceEntityId('Currency', '1');
		$data = array (
			   'subject' => $subject,
			   'quotestage'=> 'Created',
			   ...
			   'hdnDiscountAmount'=>'0',
			   'LineItems'=>$lineItem,
			   'cf_1560' => $passengerinfo,
			   'cf_1162' => $travel_Date,
			   'terms_conditions'=>$terms_conditions,
			   'adjustment'=>10,
			   'cf_1542'=>$input_array['priority']
	   );	
	   
		foreach($customerinfo as $fl=>$fv){
			if($fl == 'account_id'){
				$data[$fl] = vtws_getWebserviceEntityId('Accounts', $fv);
			}else{
				$data[$fl]=$fv;
			}
		}			
		$data['contact_id'] = vtws_getWebserviceEntityId('Contacts', $contactid);
		$quoteid = $input_array['quote_id'];
		if(!empty($quoteid)){
			$data['quotestage'] = 'Created';
			$data['id'] = vtws_getWebserviceEntityId('Quotes',$quoteid);
			$quote = vtws_update($data, $current_user);			
			$quoid = explode("x",$quote['id']);
			$output['quoteid'][] =$quoteid;
		}else{
			$data['quotestage'] = $input_array['quotestage'];
			$quote = vtws_create('Quotes', $data, $current_user);
			//$desc = vtws_retrieve($quote,$current_user);
			$quoid = explode("x",$quote['id']);
			$output['quoteid'][] = $quoid['1'];
		}	
	} catch (WebServiceException $ex) {
		$output[] = array($ex->getMessage());
	}
	$log->debug("Ended add_quote_so_inv_record ");
	return $output;
}
...
function change_password($input_array)
{
	...
	$oldpassword = $input_array['oldpassword'];
	$version = '7.0.0';

	$list = authenticate_user($username,$oldpassword,$version ,'false');
 ...
 *
 */
function get_product_list_values($id,$modulename,$only_mine='true', $category)
{
	...
	//vtiger_seproductsrel.crmid in (". generateQuestionMarks($entity_ids_list).") and, array($entity_ids_list)
	$fields_list['Related To'] = 'entityid';
	$fields_list['productcategory'] = 'productcategory';
	 
	$fields_list['imagepath'] = 'imagepath';
	$fields_list['qtyinstock'] = 'qtyinstock';
	
	$query = array();
	$params = array();
	$query = "SELECT vtiger_products.*,vtiger_crmentity.*, vtiger_productcf.cf_928, vtiger_productcf.cf_1317,vtiger_productcf.cf_1248, vtiger_seproductsrel.crmid as entityid, vtiger_seproductsrel.setype FROM vtiger_products
		...
		WHERE vtiger_crmentity.deleted = 0 AND vtiger_products.discontinued = 1 AND vtiger_productcf.cf_1562 =1 AND vtiger_productcf.cf_928 ='$category' ";
	...
	//{
		...
		for( $j= 0;$j < $noofdata; $j++)
		{
			...
			$prodcutsinfo['imagepath'] =  getImagePath($fieldid);
			$prodcutsinfo['record'] =  $fieldid;
			$prodcutsinfo['name'] =  $adb->query_result($res,$j,'productname');
			$prodcutsinfo['category'] =  $adb->query_result($res,$j,'cf_928');
			$prodcutsinfo['description'] =  $adb->query_result($res,$j,'cf_1317');
			$prodcutsinfo['city'] =  $adb->query_result($res,$j,'cf_1248');
			
			$output[$modulename][] = $prodcutsinfo;
	}
	$log->debug("Exiting function get_product_list_values.....");
	return $output;
}
 ...
 */
function get_details($id,$module)
{	
	...
	for($i=0;$i<$nooffields;$i++)
	{
			...
			|| $fieldname == 'account_id' || $fieldname == 'contact_id' || $columnname == 'linktoaccountscontacts')
		{
			...
			if ($crmid != '' && $modulename != '') {
				$fieldvalues = getEntityName($modulename, array($crmid));
				if($modulename == 'Contacts')
				$fieldvalue = $fieldvalues[$crmid];//'<a href="index.php?module=Contacts&action=index&id='.$crmid.'">'.$fieldvalues[$crmid].'</a>';
				elseif($modulename == 'Accounts')
				$fieldvalue = $fieldvalues[$crmid];//'<a href="index.php?module=Accounts&action=index&id='.$crmid.'">'.$fieldvalues[$crmid].'</a>';
				else
				$fieldvalue = $fieldvalues[$crmid];
			} else {
				$fieldvalue = '';
		...
		$output[0][$module][$fieldname] = $fieldvalue;
	}
	$output[0][$module]['record'] =  $adb->query_result($res,0,'crmid');
	$inventorymodules = array('Quotes','Invoice','SalesOrder','PurchaseOrder');


	if(in_array($module,$inventorymodules)){
		require_once 'modules/Inventory/models/Record.php';
		if($module == 'Quotes'){
			$record = $adb->query_result($res,0,'quoteid');
		}elseif($module == 'SalesOrder'){
			$record = $adb->query_result($res,0,'salesorderid');
		}elseif($module == 'PurchaseOrder'){
			$record = $adb->query_result($res,0,'purchaseorderid');
		}elseif($module == 'Invoice'){
			$record = $adb->query_result($res,0,'invoiceid');
		}
		 
		if(!empty($record)){
			require_once 'AngularUtils.php';
			$recordModel = Inventory_Record_Model::getInstanceById($record);
			if($recordModel) {
				$numOfCurrencyDecimalPlaces = getCurrencyDecimalPlaces();
				$relatedProducts = getAngularAssociatedProducts($recordModel->getModuleName(), $recordModel->getEntity());
				$productsCount = count($relatedProducts);
				$finalDetails = $recordModel->getProducts();
			}
			$relatedProducts['finalcal'] = $finalDetails[1]['final_details'];
			$output[0][$module]["Products"] = $relatedProducts;
		}
	}
	if($module == 'Products'){
		$record = $adb->query_result($res,0,'productid');
		$output[0][$module]['imagepath'] = getImagePath($record);
	}
	if($module == 'HelpDesk'){
